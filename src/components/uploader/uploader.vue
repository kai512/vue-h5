<template>
	...
	<div class="weui-cell">
		<div class="weui-cell__bd">
			<div class="weui-uploader" :class="classes">
				<div class="weui-uploader__bd" :class="{small: size === 'small'}">
					<ul class="weui-uploader__files">
						<draggable :options="sortOptions" @end="sortEnd" element="div" class="js-sort-box" v-model="imageList">
						<li class="weui-uploader__file sortable" v-for="(image, index) in imageList" @click.self="preview(image, index)" :style="{backgroundImage : `url(${image.filePath})`}" :index="index" :key="index">
							<i class="del-btn" @click="remove(index)"></i>
							<i class="re-upload-btn" v-if="!multiple && isReUpload" @click="reUpload(index)">重新上传</i>
						</li>
						<div v-show="!readonly && imageList.length < maxCount" class="weui-uploader__input-box" @click="add">
							<input v-if="!handleClick" ref="input" class="weui-uploader__input" type="file" accept="image/*" :multiple="multiple"  @change="handleChange">
						</div>
						</draggable>
					</ul>
				</div>
...
<script>
	...
	export default {
		props: {
			...
			uploadUrl: {
				type: String,
				default: config.UPLOAD_URL || `${config.serviceBaseUrl.base}/zuul/tongplatform/common/third-party-extranet/v1/attachment/upload-pics`
			},
			base64UploadUrl: {
				type: String,
...
//              			let file = this.dataURLtoFile(res.base64, res.origin.name);
			...
			sortEnd(e){
				...
				if(e.newIndex != e.oldIndex) {
					this.$emit("handle-sort-end", this.imageList, this.otherData);
        		}
			},
			...
			remove(index) {
                ...
                this.count--;
                this.$emit("remove-handle", this.imageList);
			},
			preview(image, index) {
				this.$emit('preview', image, index, this.imageList);
			}
		},
		 watch: {
        	images: {
...
.weui-uploader:not(.attachment):not(.identity-uploader){
	margin-right: -.75rem;
	margin-bottom: -.75rem;
	...
	.weui-uploader__file .re-upload-btn{
		text-align: center;
		display: block;
		position: absolute;
		left: 0;
