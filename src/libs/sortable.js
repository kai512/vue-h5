	...
	(function(factory) {
		...
		function Sortable(el, options) {
			...
			var defaults = {
				...
				scrollSpeed: 10,
				draggable: /[uo]l/i.test(el.nodeName) ? 'li' : '.sortable',
		...
		Sortable.prototype = /** @lends Sortable.prototype */ {
			...
			_emulateDragOver: function() {
				if(touchEvt) {
					...
					var parent = target
					var groupName = ' ' + this.options.group.name + ''
			...
			_onDragOver: function( /** Event */ evt) {
				...
				var options = this.options
				var group = options.group
				var groupPut = group.put
				var isOwner = (activeGroup === group)
				var canSort = options.sort

				if(evt.preventDefault !== void 0) {
					evt.preventDefault() !options.dragoverBubble && evt.stopPropagation()
				}
				...
				if(activeGroup && !options.disabled &&
					...
					if((el.children.length === 0) || (el.children[0] === ghostEl) ||
						...
						var floating = /left|right|inline/.test(lastCSS.cssFloat + lastCSS.display) ||
						...
						var moveVector = _onMove(rootEl, el, dragEl, dragRect, target, targetRect)
						var after
			...
			_onDrop: function( /** Event */ evt) {
				...
				if(evt) {
					if(moved) {
						evt.preventDefault() !options.dropBubble && evt.stopPropagation()
					}
		...
		function _cloneHide(state) {
			if(cloneEl && (cloneEl.state !== state)) {
				_css(cloneEl, 'display', state ? 'none' : '') !state && cloneEl.state && rootEl.insertBefore(cloneEl, dragEl)
				cloneEl.state = state
			}
		}
		...
		function _dispatchEvent(sortable, rootEl, name, targetEl, fromEl, startIndex, newIndex) {
			...
			var options = (sortable || rootEl[expando]).options
